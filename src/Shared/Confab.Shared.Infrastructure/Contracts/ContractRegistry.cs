...
using Confab.Shared.Abstractions.Contracts;
...
namespace Confab.Shared.Infrastructure.Contracts
{
    internal class ContractRegistry : IContractRegistry
    {
        ...
        internal void Init(IEnumerable<Assembly> assemblies)
        {
            LoadTypes(assemblies);
        }
        ...
        private void ValidateContract(Type contractType, string path = null)
        {
            ...
            if (messageAttribute is null || string.IsNullOrWhiteSpace(messageAttribute.Module) ||
                !messageAttribute.Enabled)
            {
                return;
            }
            ...
            var originalType = _types
                .Where(x => x.FullName is {} &&
                            x.FullName.Contains($"{ModulePart}.{module}", StringComparison.InvariantCultureIgnoreCase))
            ...
            _logger.LogInformation($"Validating the contract '{contractName}', " +
                             $"from module: '{contractModule}', source module: '{module}'...");
            ...
            _logger.LogInformation($"Successfully validated the contract: '{contractName}', " +
                             $"from module: '{contractModule}', source module: '{module}'.");
        }
            ...
            string propertyName, string contractName, string module, string localModule, string path = null)
        {
            ...
            throw new ContractException($"Property: '{propertyName}' in contract: '{contractName}' from module: '{localModule}', " +
                                        $"source module: '{module}'{(path is null ? "" : $", path: '{path}'")}, has a different type " +
            ...
            while (true)
            {
                ...
                if (property is null)
                {
                    throw new ContractException($"Property: '{originalName}' was not found in " +
                                                $"contract: '{contractName}' from module: '{localModule}', source module: '{module}'" +
